...
function closestPointToRect(px, py, rect) {
  return { x: clamp(px, rect.x, rect.x + rect.w), y: clamp(py, rect.y, rect.y + rect.h) };
}
...
function collisionShapesOverlap(a, b) {
  ...
  if (Array.isArray(a)) {
    return a.some((shape) => collisionShapesOverlap(shape, b));
  }

  if (Array.isArray(b)) {
    return b.some((shape) => collisionShapesOverlap(a, shape));
  }
...
function pointInCollisionShape(x, y, shape) {
  ...
  if (shape.type === 'circle') {
    return distSq(x, y, shape.x, shape.y) <= shape.radius * shape.radius;
  }
