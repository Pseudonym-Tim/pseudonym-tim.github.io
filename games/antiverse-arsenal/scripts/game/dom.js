// DOM handles...
const container = document.getElementById('game-container');
const floatingTextCanvas = document.getElementById('floating-text-overlay');
const playerHud = document.getElementById('player-hud');
const hullValue = document.getElementById('hull-label');
const hullGauge = document.getElementById('hull-gauge');
const hullGaugeNeedle = document.getElementById('hull-gauge-needle');
const stabilityValue = document.getElementById('stability-label');
const roundValue = document.getElementById('round-label');
const multiverseValue = document.getElementById('multiverse-label');
const scoreValue = document.getElementById('score-label');
const highscoreValue = document.getElementById('highscore-label');
const tunnelBackgroundCanvas = document.getElementById('tunnel-background');
const moneyValue = document.getElementById('money-label');
const nextUniverseValue = document.getElementById('next-universe-label');
const incursionValue = document.getElementById('incursion-label');
const dashCooldown = document.getElementById('dash');
const warpCooldown = document.getElementById('warp');
const laserCooldown = document.getElementById('laser');
const spawnTimerValue = document.getElementById('spawn-timer');
const spawnBanner = document.getElementById('spawn-banner');
const encounterTimeline = document.getElementById('encounter-timeline');
const focusOverlay = document.getElementById('focus-overlay');
const focusTitle = document.getElementById('focus-title');
const focusBody = document.getElementById('focus-body');
const mainMenuHighscore = document.getElementById('main-menu-highscore');
const mainMenuHint = document.getElementById('main-menu-hint');
const startGameButton = document.getElementById('start-game-button');
const mainMenuControlsButton = document.getElementById('main-menu-controls-button');
const messageOverlay = document.getElementById('message-overlay');
const messageText = document.getElementById('message-text');
const powerupOverlay = document.getElementById('powerup-overlay');
const powerupOptions = document.getElementById('powerup-options');
const shopFeedback = document.getElementById('shop-feedback');
const shopCash = document.getElementById('shop-cash');
const shopRerollButton = document.getElementById('shop-reroll-button');
const shopContinueButton = document.getElementById('shop-continue-button');
const multiverseCompleteOverlay = document.getElementById('multiverse-complete-overlay');
const multiverseCompleteTitle = document.getElementById('multiverse-complete-title');
const multiverseCompleteEarnedLabel = document.getElementById('multiverse-complete-earned-label');
const multiverseCompleteEarned = document.getElementById('multiverse-complete-earned');
const multiverseCompleteSpentLabel = document.getElementById('multiverse-complete-spent-label');
const multiverseCompleteSpent = document.getElementById('multiverse-complete-spent');
const multiverseCompleteNetLabel = document.getElementById('multiverse-complete-net-label');
const multiverseCompleteNet = document.getElementById('multiverse-complete-net');
const multiverseCompleteWrapHitsLabel = document.getElementById('multiverse-complete-wrap-hits-label');
const multiverseCompleteWrapHits = document.getElementById('multiverse-complete-wrap-hits');
const multiverseCompleteBestWrapLabel = document.getElementById('multiverse-complete-best-wrap-label');
const multiverseCompleteBestWrap = document.getElementById('multiverse-complete-best-wrap');
const multiverseCompleteContinueButton = document.getElementById('multiverse-complete-continue-button');
const pauseOverlay = document.getElementById('pause-overlay');
const pauseTitle = document.getElementById('pause-title');
const resumeButton = document.getElementById('resume-button');
const controlsButton = document.getElementById('controls-button');
const controlsPanel = document.getElementById('controls-panel');
const controlsCloseButton = document.getElementById('controls-close-button');
const controlsBackButton = document.getElementById('controls-back-button');
const pauseRestartButton = document.getElementById('pause-restart-button');
const quitButton = document.getElementById('quit-button');
const gameoverOverlay = document.getElementById('gameover-overlay');
const gameoverCauseEl = document.getElementById('gameover-cause-label');
const gameoverAbilityEl = document.getElementById('gameover-ability-label');
const gameoverLeastAbilityEl = document.getElementById('gameover-least-ability-label');
const finalScoreEl = document.getElementById('final-score-label');
const finalHighscoreEl = document.getElementById('final-highscore-label');
const finalMultiverseEl = document.getElementById('final-multiverse-label');
const finalWrapHitsEl = document.getElementById('final-wrap-hits-label');
const finalHighestWrapShotEl = document.getElementById('final-highest-wrap-shot-label');
const restartButton = document.getElementById('restart-button');